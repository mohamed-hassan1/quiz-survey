(function () {
  ...
  let globalFun = {
    ...
    heightAnimation: function (max, min, dir, ele, count, speed, callback) {
  ...
  window.addEventListener("click", function (e) {
    ...
    if (
      !globalFun.closest(e.target, "drop-menu") &&
      !globalFun.closest(e.target, "dropdown") &&
      document.querySelector(".drop-menu.active")
    ) {
  ...
  function fullHeight() {
    ...
    if (innerInput && window.innerHeight > 660) {
      innerInput.style.marginBottom =
        globalFun.animationSpeed([80, 100, 130, 180], null) + "px";
    } else if (innerInput && window.innerHeight < 660) {
      innerInput.style.marginBottom =
        globalFun.animationSpeed([80, 100, 180, 180], null) + "px";
    }
  }
  ...
  questionSection.addEventListener("input", function (e) {
    if (this.classList.contains("active")) {
      if (e.target.classList.contains("txtholder")) {
        // remove valid class
        if (
          globalFun.closest(e.target, "question").classList.contains("valid")
        ) {
        ...
        if (e.target.parentElement.classList.contains("placeholder")) {
          if (e.target.value !== "") {
            e.target.parentElement.querySelector("label").style.display =
              "none";
          } else {
            e.target.parentElement.querySelector("label").style.display =
              "block";
        ...
        if (e.target.classList.contains("answer-input")) {
          let inputs = globalFun
              .closest(e.target, "input-container")
            ...
            for (let a = 0; a < inputs.length; a++) {
              if (inputs[a].value !== "") {
                if (inputs[a].hasAttribute("data-length")) {
                  if (
                    Number(inputs[a].getAttribute("data-length")) <=
                    inputs[a].value.length
                  ) {
                    num++;
  ...
  window.addEventListener("keypress", function (e) {
    if (e.code === "Enter" || e.code == 13) {
      if (
        document.querySelector(".questions-section.active") &&
        document.querySelector(".accept-btn-container.active")
      ) {
        acceptBtnFun();
      }
    }
      ...
      if (arrow.classList.contains("active")) {
        if (arrow.classList.contains("next")) {
          ...
          if (document.querySelector(".accept-btn-container.active")) {
            ...
            if (
              document.querySelector(
                ".question.active .question-content.choose"
              ) &&
              document.querySelector(
                ".question.active .question-content.choose .choose-lbl.active"
              )
            ) {
              acceptBtnFun();
            }
  ...
  let multiChoices = document.querySelectorAll(
    ".question-content.choose .input-container"
  );
  if (multiChoices[0]) {
  ...
  function questionAnimation(ques, status) {
    let frontSectionHeight =
        document.querySelector(".front-section").offsetHeight,
      ...
      inputContainer = document.querySelector(
        ".question.ready .input-container"
      ),
      accTopBtn = quesHeight * 0.5 + 20;
    if (inputContainer && inputContainer.classList.contains("drop-menu")) {
      accTopBtn =
        accTopBtn - Number(inputContainer.style.marginBottom.replace("px", ""));
    }
    ...
    if (status === "next") {
      progressBarFun(true);
      curStyle.innerHTML =
        ".questions-section .accept-btn-container.fade {opacity: 0;top: calc(50% + " +
        (accTopBtn + 30) +
        "px); animation: acceptBtnFade .25s ease-in-out;} .questions-section .accept-btn-container.active {top:calc(50% + " +
        accTopBtn +
        "px)}.questions-section .question.active {animation: questionAnimation .4s ease-in-out;} @keyframes questionAnimation {0% {top:" +
        quesNum +
        "px; opacity: 0} 100% {top: 50%; opacity: 1;}} @keyframes acceptBtn {0% {top: calc(50% + " +
        (accTopBtn + 30) +
        "px); opacity: 0;} 100% {top: calc(50% + " +
        accTopBtn +
        "px); opacity: 1;}} @keyframes acceptBtnFade {0% {top: calc(50% + " +
        accTopBtn +
        "px); opacity: 1;} 100% {top: calc(50% + " +
        (accTopBtn + 30) +
        "px); opacity: 0;}}";
      ...
      curStyle2.innerHTML =
        ".questions-section .question.done {animation: questionAnimation2 .75s ease-in-out;} @keyframes questionAnimation2 {0% {top: 50%; opacity: 1} 100% {top:-" +
        quesNum +
        "px; opacity: 0;}}";
      ...
      let quesNum2 = prevQues.offsetHeight * 0.5,
        contentHeight =
          document.querySelector(".questions-section .content").offsetHeight *
          0.5;
      ...
      if (inputContainer) {
        accTopBtn =
          accTopBtn -
          Number(inputContainer.style.marginBottom.replace("px", ""));
      }
      ...
      curStyle3.innerHTML =
        ...
        "px); animation: acceptBtnFade .25s ease-in-out;} .questions-section .accept-btn-container.active {top:calc(50% + " +
        accTopBtn +
        "px)}  .questions-section .question.ready2 {animation: questionAnimation4 .75s ease-in-out;} .questions-section .question.active2 {animation: questionAnimation3 .5s ease-in-out;} @keyframes questionAnimation3 {0% {top:-" +
        quesNum2 +
        "px; opacity: 0;} 100% {top: 50%; opacity: 1;}} @keyframes questionAnimation4 {0% {top: 50%; opacity: 1;} 100% {top:" +
        quesNum +
        "px; opacity: 0;}} @keyframes acceptBtn {0% {top: calc(50% + " +
        (accTopBtn + 30) +
        "px); opacity: 0;} 100% {top: calc(50% + " +
        accTopBtn +
        "px); opacity: 1;}} @keyframes acceptBtnFade {0% {top: calc(50% + " +
  ...
  function startQuestions() {
    ...
    if (!questionsSection.classList.contains("active")) {
      ...
      if (
        currQuestion.nextElementSibling &&
        currQuestion.nextElementSibling.classList.contains("question")
      ) {
  ...
  function progressBarFun(status) {
    ...
    if (progressBar.classList.contains("active")) {
      ...
      innerBar.style.width =
        Number(innerBar.getAttribute("data-progress")) + "px";
      ...
      if (status) {
        ...
        if (percent + Number(percentageNum.textContent) > 100) {
          ...
          percentageNum.textContent =
            Number(percentageNum.textContent) + percent;
        }
      } else {
        // back
        num = innerBarWidth - num;
      ...
      style.innerHTML =
        ".questions-section .bar .inner-bar {width: " +
        num +
        "px; animation: progressBarAnim .25s ease-in-out;} @keyframes progressBarAnim {0% {width: " +
        innerBarWidth +
        "px;} 100% {width: " +
        num +
        "px;}}";
